...
const DirectorDashboard = () => {
    ...
    const handleAction = async (e) => {
        ...
        try {
            ...
            if (actionType === 'validate') {
                await requestService.directorValidate(reqId, reason);
            } else if (actionType === 'block') {
                await requestService.directorBlock(reqId, reason);
            }
    ...
    const handleSaveItems = async (e) => {
        ...
        try {
            ...
            await requestService.updateDirectorItems(selectedRequest.id, validItems, modificationReason);
    ...
    const getStatusBadge = (status) => {
        ...
        const statusMap = {
            ...
            '4': { class: 'status-info', label: 'Validé' },
            'DirectorValidated': { class: 'status-info', label: 'Validé' },
            '5': { class: 'status-warning', label: 'Modifié' },
            'DirectorModified': { class: 'status-warning', label: 'Modifié' },
            '6': { class: 'status-rejected', label: 'Bloqué' },
            'DirectorBlocked': { class: 'status-rejected', label: 'Bloqué' },
    ...
    const getActions = (req) => {
        ...
        // Only show actions for requests that have been validated by Control
        if (status === 'ControlValidated') {
        ...
        // Allow validation/blocking for requests that were modified by Director
        if (status === 'DirectorModified' || status === 5) {
            actions.push({ type: 'validate', label: '✓ Valider', class: 'btn-approve' });
            actions.push({ type: 'block', label: '✕ Bloquer', class: 'btn-reject' });
        }
        return actions;
                        ...
                        <form onSubmit={handleSaveItems}>
                            ...
                            <div className="form-group" style={{marginTop: '15px'}}>
                                ...
                                <textarea
                                    ...
                                    required
                                    placeholder="Expliquer le motif de modification..."
                                />
                            </div>
                            
                            <div className="modal-actions">
                                <button type="submit" className="btn-approve">Enregistrer</button>
                                <button type="button" onClick={() => setShowItemsModal(false)} className="btn-secondary">Annuler</button>
                            </div>
                        </form>
                    </div>
                </div>
